...
public class ReturnOrderServiceImpl implements ReturnOrderService {
    ...
    @Override
    @Transactional
    public void saveSales(ReturnOrder returnOrders) {

        returnOrders.setId(idWorker.nextId()); //设置分布式Id
        returnOrders.setApplyTime(new Date());//申请时间
        if ("1".equals(returnOrders.getType())) {
            returnOrders.setStatus("0");//申请状态

            //通过退货订单id修改订单明细表的退货状态
            OrderItem orderItem = orderItemMappers.selectByPrimaryKey(returnOrders.getOrderId());
            orderItem.setIsReturn("1");//修改退货状态(已申请退货)

            returnOrderMapper.insert(returnOrders);
            orderItemMappers.updateByPrimaryKey(orderItem);

        }else {  //退款
            returnOrders.setStatus("0");//申请状态
            //通过退货订单id修改订单明细表的退款状态
            OrderItem orderItem = orderItemMappers.selectByPrimaryKey(returnOrders.getOrderId());
            orderItem.setIsReturn("0");//修改退款状态(未退款)
