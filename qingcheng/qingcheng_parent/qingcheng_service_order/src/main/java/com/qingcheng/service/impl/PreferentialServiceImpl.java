...
public class PreferentialServiceImpl implements PreferentialService {
     ...
     */
    @Override
    public int findByPreMoneyByCategoryId(Long categoryId, int money) {
        ...
        criteria.andLessThanOrEqualTo("buyMoney", money); //购买金额小于等于消费金额
        ...
        //如果存在该满减规则
        if (preferentials.size() >= 1 && preferentials != null) {
            ...
            if ("1".equals(preferential.getType())) {
                ...
                //翻倍
                int multNum = money / preferential.getBuyMoney(); //获取倍数
                int preMoney = preferential.getPreMoney() * multNum; //优惠总金额
                return preMoney;
            }
        } else {
            return 0; //不存在满减
